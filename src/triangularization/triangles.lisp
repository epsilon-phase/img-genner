  ...
  (flet ((dx (a b)
    ...
    (let ((p (copy-list polygon)))
      ...
      (loop for a in polygon
            for b in (cdr p)
            for c in (cddr p)
            for dx1 = (dx a b) then (dx a b)
            for dx2 = (dx b c) then (dx b c)
            for dy1 = (dy a b) then (dy a b)
            for dy2 = (dy b c) then (dy b c)
            with sign = nil
            unless sign do (setf sign (signum (zcross dx1 dy1 dx2 dy2)))
              thereis (not (= sign (signum (zcross dx1 dy1 dx2 dy2))))
            )
      ))
  )
