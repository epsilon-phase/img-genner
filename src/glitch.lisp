...
(declaim (ftype (function (#1=(simple-array (unsigned-byte 8)) #1#)
                          (values boolean))
                compare-colors-magnitude))
...
(defun central-pixel-sort (image cx cy
  ...
  (loop for y from 0 below (array-dimension image 0)
        do(loop for i in (split-n-length (line-index-interpolator 0 y cx cy) segment-length)
                do(sort-along-line image i comparison)
                )
        do(loop for i in (split-n-length (line-index-interpolator (1- (array-dimension image 1)) y cx cy) segment-length)
                do(sort-along-line image i comparison))
        )
  (loop for x from 0 below (array-dimension image 1)
        do(loop for i in (split-n-length (line-index-interpolator x 0 cx cy) segment-length)
                do(sort-along-line image i comparison)
                )
        do(loop for i in (split-n-length (line-index-interpolator x (1- (array-dimension image 0))  cx cy) segment-length)
                do(sort-along-line image i comparison)
                )
        )
  )
